...
class StepType(str, Enum):
    ...
    LOOP = "loop"
    BRANCH = "branch"
    SCRIPT = "script"  # Run a user-supplied script
    CUSTOM = "custom"
...
def validate_pipeline(spec: PipelineSpec) -> list[str]:
    ...
    # Verify generated_by references match creates_assets (auto-link if unset)
    for name, collection in spec.asset_collections.items():
        if collection.generated_by:
            ...
            if creating_step.creates_assets != name:
                if creating_step.creates_assets is None:
                    creating_step.creates_assets = name
                    warnings.append(
                        f"Auto-linked: step '{collection.generated_by}' will populate "
                        f"collection '{name}' (from generated_by). "
                        f"Add 'creates_assets: {name}' to the step to silence this."
                    )
                else:
                    raise ValidationError(
                        f"Collection '{name}' has generated_by='{collection.generated_by}', "
                        f"but that step already has creates_assets='{creating_step.creates_assets}'. "
                        f"A step can only populate one collection."
                    )
    
    # Validate for_each steps that depend on generated collections
    # have the generating step in their (transitive) dependency chain
    generated_collections: dict[str, str] = {}  # collection_name -> generating step_id
    for name, collection in spec.asset_collections.items():
        if collection.generated_by:
            generated_collections[name] = collection.generated_by
    
    def get_transitive_deps(step_id: str, _seen: set[str] | None = None) -> set[str]:
        if _seen is None:
            _seen = set()
        if step_id in _seen:
            return _seen
    ...
    for step in spec.steps:
        ...
        if collection_name in ("asset", "item"):
            collection_name = next(iter(spec.asset_collections), None)
        if collection_name and collection_name in generated_collections:
            gen_step_id = generated_collections[collection_name]
            transitive = get_transitive_deps(step.id)
            if gen_step_id not in transitive:
                raise ValidationError(
                    f"Step '{step.id}' iterates over collection '{collection_name}' "
                    f"(generated by '{gen_step_id}'), but doesn't depend on it. "
                    f"Add '{gen_step_id}' to the step's requires list."
                )
    
    # Validate template references in step configs point to real steps
    template_pattern = re.compile(r'\{([a-zA-Z_][a-zA-Z0-9_]*)\.([a-zA-Z0-9_.]+)\}')
    builtin_namespaces = {"context", "ctx", "asset", "step_outputs"}
    
    def extract_template_refs(data: Any) -> list[str]:
        """Recursively extract {namespace.field} namespaces from config values."""
        refs: list[str] = []
        if isinstance(data, str):
            refs.extend(m.group(1) for m in template_pattern.finditer(data))
    ...
    for step in spec.steps:
        if not step.config:
            continue
        refs = extract_template_refs(step.config)
        transitive = get_transitive_deps(step.id)
        for ref in refs:
            if ref in builtin_namespaces:
                continue
            if ref not in spec.step_index:
                raise ValidationError(
                    f"Step '{step.id}' references '{{{ref}.…}}' in its config, "
                    f"but no step with id '{ref}' exists."
                )
            if ref not in transitive:
                raise ValidationError(
                    f"Step '{step.id}' references '{{{ref}.…}}' in its config, "
                    f"but doesn't depend on it (directly or transitively). "
                    f"Add '{ref}' to the step's requires list."
                )
